/*
 * TDOT Roadway Cameras
 *
 * TDOT Roadway Cameras.
 *
 * OpenAPI spec version: 1.0.0
 * Contact: escofieldpublic@gmail.com
 * Generated by: https://github.com/swagger-api/swagger-codegen.git
 */

using System;
using System.Linq;
using System.IO;
using System.Text;
using System.Collections;
using System.Collections.Generic;
using System.Collections.ObjectModel;
using System.Runtime.Serialization;
using Newtonsoft.Json;

namespace IO.Swagger.Models
{

    /// <summary>
    /// 
    /// </summary>
    [DataContract]
    public partial class Camera :  IEquatable<Camera>
    {
        /// <summary>
        /// Initializes a new instance of the <see cref="Camera" /> class.
        /// </summary>
        /// <param name="Id">Id.</param>
        /// <param name="ThumbnailUrl">ThumbnailUrl.</param>
        /// <param name="Title">Title.</param>
        /// <param name="Jurisdiction">Jurisdiction.</param>
        /// <param name="Route">Route.</param>
        /// <param name="HttpVideoUrl">HttpVideoUrl.</param>
        public Camera(int? Id = default(int?), string ThumbnailUrl = default(string), string Title = default(string), string Jurisdiction = default(string), string Route = default(string), string HttpVideoUrl = default(string))
        {
            this.Id = Id;
            this.ThumbnailUrl = ThumbnailUrl;
            this.Title = Title;
            this.Jurisdiction = Jurisdiction;
            this.Route = Route;
            this.HttpVideoUrl = HttpVideoUrl;
            
        }

        /// <summary>
        /// Gets or Sets Id
        /// </summary>
        [DataMember(Name="id")]
        public int? Id { get; set; }
        /// <summary>
        /// Gets or Sets ThumbnailUrl
        /// </summary>
        [DataMember(Name="thumbnailUrl")]
        public string ThumbnailUrl { get; set; }
        /// <summary>
        /// Gets or Sets Title
        /// </summary>
        [DataMember(Name="title")]
        public string Title { get; set; }
        /// <summary>
        /// Gets or Sets Jurisdiction
        /// </summary>
        [DataMember(Name="jurisdiction")]
        public string Jurisdiction { get; set; }
        /// <summary>
        /// Gets or Sets Route
        /// </summary>
        [DataMember(Name="route")]
        public string Route { get; set; }
        /// <summary>
        /// Gets or Sets HttpVideoUrl
        /// </summary>
        [DataMember(Name="httpVideoUrl")]
        public string HttpVideoUrl { get; set; }

        /// <summary>
        /// Returns the string presentation of the object
        /// </summary>
        /// <returns>String presentation of the object</returns>
        public override string ToString()
        {
            var sb = new StringBuilder();
            sb.Append("class Camera {\n");
            sb.Append("  Id: ").Append(Id).Append("\n");
            sb.Append("  ThumbnailUrl: ").Append(ThumbnailUrl).Append("\n");
            sb.Append("  Title: ").Append(Title).Append("\n");
            sb.Append("  Jurisdiction: ").Append(Jurisdiction).Append("\n");
            sb.Append("  Route: ").Append(Route).Append("\n");
            sb.Append("  HttpVideoUrl: ").Append(HttpVideoUrl).Append("\n");
            sb.Append("}\n");
            return sb.ToString();
        }

        /// <summary>
        /// Returns the JSON string presentation of the object
        /// </summary>
        /// <returns>JSON string presentation of the object</returns>
        public string ToJson()
        {
            return JsonConvert.SerializeObject(this, Formatting.Indented);
        }

        /// <summary>
        /// Returns true if objects are equal
        /// </summary>
        /// <param name="obj">Object to be compared</param>
        /// <returns>Boolean</returns>
        public override bool Equals(object obj)
        {
            if (ReferenceEquals(null, obj)) return false;
            if (ReferenceEquals(this, obj)) return true;
            if (obj.GetType() != GetType()) return false;
            return Equals((Camera)obj);
        }

        /// <summary>
        /// Returns true if Camera instances are equal
        /// </summary>
        /// <param name="other">Instance of Camera to be compared</param>
        /// <returns>Boolean</returns>
        public bool Equals(Camera other)
        {

            if (ReferenceEquals(null, other)) return false;
            if (ReferenceEquals(this, other)) return true;

            return 
                (
                    this.Id == other.Id ||
                    this.Id != null &&
                    this.Id.Equals(other.Id)
                ) && 
                (
                    this.ThumbnailUrl == other.ThumbnailUrl ||
                    this.ThumbnailUrl != null &&
                    this.ThumbnailUrl.Equals(other.ThumbnailUrl)
                ) && 
                (
                    this.Title == other.Title ||
                    this.Title != null &&
                    this.Title.Equals(other.Title)
                ) && 
                (
                    this.Jurisdiction == other.Jurisdiction ||
                    this.Jurisdiction != null &&
                    this.Jurisdiction.Equals(other.Jurisdiction)
                ) && 
                (
                    this.Route == other.Route ||
                    this.Route != null &&
                    this.Route.Equals(other.Route)
                ) && 
                (
                    this.HttpVideoUrl == other.HttpVideoUrl ||
                    this.HttpVideoUrl != null &&
                    this.HttpVideoUrl.Equals(other.HttpVideoUrl)
                );
        }

        /// <summary>
        /// Gets the hash code
        /// </summary>
        /// <returns>Hash code</returns>
        public override int GetHashCode()
        {
            // credit: http://stackoverflow.com/a/263416/677735
            unchecked // Overflow is fine, just wrap
            {
                int hash = 41;
                // Suitable nullity checks etc, of course :)
                    if (this.Id != null)
                    hash = hash * 59 + this.Id.GetHashCode();
                    if (this.ThumbnailUrl != null)
                    hash = hash * 59 + this.ThumbnailUrl.GetHashCode();
                    if (this.Title != null)
                    hash = hash * 59 + this.Title.GetHashCode();
                    if (this.Jurisdiction != null)
                    hash = hash * 59 + this.Jurisdiction.GetHashCode();
                    if (this.Route != null)
                    hash = hash * 59 + this.Route.GetHashCode();
                    if (this.HttpVideoUrl != null)
                    hash = hash * 59 + this.HttpVideoUrl.GetHashCode();
                return hash;
            }
        }

        #region Operators
        /// Equality operator
        public static bool operator ==(Camera left, Camera right)
        {
            return Equals(left, right);
        }
        /// Inequality operator
        public static bool operator !=(Camera left, Camera right)
        {
            return !Equals(left, right);
        }

        #endregion Operators

    }
}
